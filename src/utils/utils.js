...
const formatPace = (d) => {
    ...
    return `${minutes.toFixed(0).toString().padStart(2, '0')}:${seconds.toFixed(0).toString().padStart(2, '0')}`;
};

const formatAvgPace = (p) => {
    const minutes = Math.floor(p);
    const seconds = Math.floor((p - minutes) * 60.0);
    return `${minutes.toFixed(0).toString().padStart(2, '0')}:${seconds.toFixed(0).toString().padStart(2, '0')}`;
};

const convertToSecond = (t) => {
    const ta = t.split(':');
    var sec = 0;
    if (ta.length >= 3) {
        sec = Number(ta[0] * 3600) + Number(ta[1] * 60) + Number(ta[2]);
    } else if (ta.length == 2) {
        sec = Number(ta[0] * 60) + Number(ta[1]);
    } else {
        sec = Number(t);
    }
    return sec;
...
const formatShowTime = (t) => {
    const sec = convertToSecond(t)
    return formatSecondTime(sec);
}

const formatSecondTime = (total_sec) => {
    const sec = total_sec % 60;
    const min = Math.floor(total_sec % 3600 / 60);
    const hour = Math.floor(total_sec % 86400 / 3600);
    var out = `${min.toFixed(0).toString().padStart(2, '0')}:${sec.toFixed(0).toString().padStart(2, '0')}`;
    if (hour > 0) {
        out = `${hour.toFixed(0).toString().padStart(2, '0')}:${out}`;
    }
    return out;
}
...
const formatRunTime = (distance, pace) => {
    ...
    const total_sec = Math.floor(pace * 60 * distance);
    return formatSecondTime(total_sec);
};
